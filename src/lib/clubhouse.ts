...
function createSeedStore(): ClubhouseStore {
  ...
  const games = [
    createGame("NBA", "Boston Celtics", "New York Knicks", 2, 4.5, -110),
    createGame("NHL", "Toronto Maple Leafs", "New Jersey Devils", 4, 1.5, -108),
    createGame("MLB", "Los Angeles Dodgers", "Atlanta Braves", 6, 1.5, -115),
    createGame("NCAAB", "Duke", "North Carolina", 8, 3.5, -110),
    createGame("WNBA", "Las Vegas Aces", "New York Liberty", 10, 2.5, -112),
  ];
  ...
  const slips: BetSlipView[] = [
      ...
      legs: [
        {
          id: "leg_demo_1",
          gameId: "finished_nba",
          selectionId: "finished_nba:home",
          selectionTeam: "Miami Heat",
          selectionSide: "home",
          spread: -2.5,
          americanOdds: -110,
          bookmaker: defaultSettings.primaryBookmaker,
    ...
    },
    {
      ...
      type: "parlay",
      stakeCents: 1500,
      potentialPayoutCents: calculateParlayPayout(1500, [-110, -108]),
      payoutCents: 0,
      status: "open",
  ...
  const lockPicks: LockPickView[] = [
    {
      id: "lock_demo_1",
      userId: "user_rome",
      gameId: games[2]?.id ?? "demo_game_1",
      weekKey: currentWeekKey(),
      selectionId: games[2]?.options[0]?.id ?? "",
      selectionTeam: games[2]?.options[0]?.team ?? "Los Angeles Dodgers",
      selectionSide: games[2]?.options[0]?.side ?? "away",
      spread: games[2]?.options[0]?.spread ?? 1.5,
      americanOdds: games[2]?.options[0]?.americanOdds ?? -115,
      bookmaker: games[2]?.options[0]?.bookmaker ?? defaultSettings.primaryBookmaker,
      quoteTimestamp: games[2]?.options[0]?.quoteTimestamp ?? nowIso(),
      result: "pending",
  ...
  const activity: ActivityItem[] = [
    {
      id: makeId("activity"),
      message: "Rome cashed a Heat cover and jumped to the top of the board.",
      createdAt: addHours(new Date(), -4).toISOString(),
      tone: "good",
    },
    {
      id: makeId("activity"),
      message: "Smoke built a two-leg heater for tonight's card.",
      createdAt: addHours(new Date(), -2).toISOString(),
      tone: "neutral",
    },
    {
      id: makeId("activity"),
      message: "Jules requested a bankroll top-up for the weekend slate.",
